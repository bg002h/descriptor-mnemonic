...
- **A second taproot internal-key kind, at wire version 8 (F-449 stage 1b).**
  Liana computes its "unspendable" internal key by deriving it from the
  tap-tree's own leaf keys (its own recipe, not the literal BIP-341 NUMS
  H-point), and until now recognising that convention meant re-deriving
  Liana's own function over the whole descriptor — a coordinator
  computation the codec could not see. A single new kind bit, legal only at
  wire version 8 (version 4 payloads are unchanged and still mean "NUMS"
  when the existing `is_nums` bit is set), gives the convention its own
  wire discriminant: `InternalKey::LianaUnspendable`, distinct from
  `InternalKey::NumsPoint`.

  - `md decompose` recognises a real Liana unspendable internal key and
    gives it no placeholder slot — the same treatment NUMS already gets.
  - `md encode` accepts the `UNSPENDABLE(liana)` marker in a template's
    internal-key position and derives the key per SPEC §2, rather than
    taking it as a literal value.
  - Four encode-side refusals stop a kind-1 card being minted in a shape
    Liana itself cannot import, or that the wire cannot express minimally:
    a `sortedmulti_a` tap leaf under a kind-1 key (§2's recipe hashes
    leaves in unsorted wire order, so a `sortedmulti_a` leaf is exactly the
    shape a sorting port error would silently diverge on), a non-canonical
    `<0;1>/*` use-site (Liana pairs multipath alternatives positionally),
    a kind-1 key found on a nested `tr()` rather than the descriptor's own
    root, and a wire version requested higher than the tree actually needs.
  - A kind-0 (NUMS) and a kind-1 (Liana unspendable) policy that are
    otherwise identical now get **different wallet identities**: the kind
    bit is inside what `compute_wallet_descriptor_template_id` hashes, so a
    card minted for one never matches a card minted for the other.

### Changed

- **BREAKING, Rust API only — zero wire bytes changed (F-449 stage 1a).**
  `Body::Tr`'s shape changed ahead of the feature above: the former
  `is_nums: bool` + `key_index: u8` pair — whose invariant ("`is_nums =
  true` implies `key_index = 0`") was enforced only by a `debug_assert` —
  is replaced by a single `internal_key: InternalKey` field, where
  `InternalKey` is a new public enum (`Slot(u8)`, `NumsPoint`, and the
  `LianaUnspendable` this release makes reachable). Every wire-version-4
  payload still encodes to byte-identical bytes; a golden captured from the
  unmodified code proves it. Callers matching `Body::Tr { is_nums,
  key_index, .. }` directly must switch to matching `internal_key` instead.
...
- **Host seating fills an ABSENT slot fingerprint from the card that seats
  it.** **The completed wallet's id changes from what 0.16.x produced** for any
  partial template — a template minted with an unseated slot (SPEC §8p) and
  completed on the host. Addresses do NOT change (origin metadata is not script
  content); measured across every seating fixture, five change their composed
  wallet id and all five keep byte-identical addresses.

  Before this, `seat::compose` filled only `tlv.pubkeys` and inherited a
  fingerprint-free declaration as *"the policy author's choice not to state
  one"*. That conflated two things. A composer template with an unseated slot
  has no fingerprint there because the key was not known — nobody chose — and
  the host is the moment it becomes known.

  It is not cosmetic. `to_miniscript` drops the WHOLE origin when the
  fingerprint is absent, so a host-completed partial template rendered its
  completed key as a bare xpub with **no `[fp/path]` at all** — a BIP-380 key
  no coordinator and no HWI can attribute to a signer — and the wallet id
  differed from the one the device mints for the same three keys seated
  on-device. Composer fable review r0, lens 3, I-2, case C13. SPEC §5 l.218:
  *"EVERY slot declares an origin (§4f) and, when seated, the master
  fingerprint of the seated key."*

  **Unchanged, and tested:** a fingerprint the policy card DECLARES is never
  overwritten (A2 refuses a disagreeing card before `compose` sees it, and the
  unit test drives the assignment A2 would reject), and a privacy-preserving
  card that states no master leaves its slot fingerprint-free — nothing is
  invented.

  Fixture `tests/fixtures/seating/v-partial-c13.txt`, generated by
  `generate.sh`, reproduces the report's own template stub `b02b4403`.

  **Migration -- what an operator with an already-completed 0.16.x card sees
  (measured, fold-A review I-1):** cards already engraved still decode and
  still report SPEND-EQUAL under `md descriptor --verify-against` (exit 0),
  and the POLICY SHAPE id does not move, so SHAPE-confirmed stubs are
  unaffected. Only mk1 key cards stubbed with the *composed wallet id* of a
  host-completed partial template are affected: seated against the template
  they now print "stub matches neither this policy's shape id nor the
  composed wallet id -- minted under different origin metadata (legitimate),
  or a different wallet; verify address 0 before trust" instead of
  WALLET-CONFIRMED, until they are re-minted at the new id. The direction is
  fail-safe: a good set loses a confirmation, a bad one never gains one.

  One more consequence, at the wire's edge (fold-A review M-3): the fill adds
  a fingerprint entry per slot, so a completed card grows by up to one chunk
  (measured `n=6: 11 -> 12`, `n=9: 17 -> 18`, `n=11: 20 -> 21`; `n=3`
  unchanged). A partial template that sat at 63-64 chunks under 0.16.x can now
  cross the 64-chunk cap and refuse with `TooManyChunks` where it used to
  emit. Fail-safe, and inherent: the bytes are what make the key attributable.
