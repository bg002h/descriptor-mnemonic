...
///
/// The `input_strings` are either raw WDM strings (for decode-level errors)
/// or synthetic byte sequences that trigger bytecode rejections.
///
/// For conformance.rs tests that require constructing input at runtime (e.g.,
/// `rejects_mixed_case` which encodes a valid policy first), we store a
/// placeholder input that demonstrates the error class. All placeholder
/// inputs are confirmed to trigger the correct variant against a live
/// implementation.
...
fn build_positive_vectors() -> Vec<Vector> {
    ...
    for &(id, description, policy_str) in CORPUS_FIXTURES {
        ...
        let expected_bytecode_hex: String = bytecode.iter().map(|b| format!("{b:02x}")).collect();
