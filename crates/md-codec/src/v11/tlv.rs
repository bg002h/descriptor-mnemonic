...
impl TlvSection {
    ...
    pub fn write(&self, w: &mut BitWriter, key_index_width: u8) -> Result<(), V11Error> {
...
fn read_use_site_overrides(
    ...
    while r.bit_position_for_test() - start < bit_len {
        ...
        if let Some(prev) = last_idx {
            if idx <= prev {
                return Err(V11Error::OverrideOrderViolation { prev, current: idx });
            }
        }
        last_idx = Some(idx);
    ...
    if entries.is_empty() {
        return Err(V11Error::EmptyTlvEntry { tag: TLV_USE_SITE_PATH_OVERRIDES });
    }
    Ok(entries)
}
...
fn read_fingerprints(
    ...
    if entries.is_empty() {
        return Err(V11Error::EmptyTlvEntry { tag: TLV_FINGERPRINTS });
    }
    Ok(entries)
}
