...
| `5` | `REPAIR_APPLIED` — at least one chunk corrected; stdout = repair report + corrected chunks. Consistent across all four CLIs per plan D26 (`mnemonic` / `mk` / `ms` / `md`). **Since md-cli 0.19.0, `md repair` also exits 5 when the corrected card carries a wire version this build cannot read** (stderr names the version and the accepted set); `mnemonic repair` still exits 2 on such a card until it adopts `md_codec::correct_chunks` (F-642). |
