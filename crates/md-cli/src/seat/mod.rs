...
//! descriptor and addresses cells, and the whole D row. One cell this cycle
//! owned did NOT close, and is left ✗ carrying its measured reason:
//! **S → keyed card**. `md encode --key` admits only a depth-3/4 xpub, and a
//! descriptor composed from mk1 cards carries depth-0 keys (md rebuilds them
//! from the 65-byte TLV, which has no depth), so the bridge refuses — filed as
