...
**Status:** R0-GATE GREEN (R0 RED 0C/4I/4M → R1 0C/2I/1M → R2 GREEN). Cleared for the implementation plan. Reviews: `design/agent-reports/md-test-hardening-spec-R{0,...}-review.md`.
...
- **Varied params (R0 I3 — `n ∈ 1..=32`, NOT ≤15):** `n ∈ 1..=32` (`KeyCountOutOfRange` gate at `origin_path.rs:111`), with the distribution biased to the kiw-width boundaries **{1,2,3,4,5,8,9,15,16,17,31,32}** (`encode.rs:37-41` ⟷ `decode.rs:26` duplicated `⌈log₂n⌉`) — the {15,16,17,31,32} values exercise the kiw=4→5 transition that {1..9} never reaches. **Separately**, each `OriginPath.components.len() ≤ 15` (`MAX_PATH_COMPONENTS`, `origin_path.rs:43` — a DIFFERENT limit; the two were conflated). Multi-family `count` and any `Divergent` vector are capped at 32 = `n`. Also varied: `k`; **non-canonical index orderings** (emit `MultiKeys` indices like `[2,0,1]`); `path_decl` ∈ {elided-when-canonical, Shared, Divergent(len==n)}; `use_site_path` ∈ {standard_multipath, 2..=9-alt multipath} (`use_site_path.rs:43-45`); TLV maps ∈ {none, fingerprints, valid-pubkey wallet-policy, origin overrides}.
- **Pubkeys derived from a fixed valid compressed key** (e.g. secp G), NOT random `[u8;65]` (`validate_xpub_bytes` rejects ~50% otherwise, `validate.rs:221`).
- **Placeholder coverage is airtight by construction (R0 I2):** `n` is DERIVED from the distinct placeholder set actually emitted into the tree (keypath index ∪ all leaf `KeyArg`/`MultiKeys` indices), renumbered to `0..n` — NOT chosen independently. `canonicalize_placeholder_indices` errors with `PlaceholderNotReferenced` (`canonicalize.rs:184`) if any `@i` in `0..n` is unreferenced, so the **recursive taptree arm in particular** must guarantee ≥1 key-bearing leaf per declared `@i` (timelock/hash leaves like `older`/`after`/`sha256` reference no placeholder and must not dilute coverage). For the flat multisig templates this is automatic (indices = `0..n`); the canonicalization in P1 then absorbs any deliberate permutation.
...
### §3.2 Properties
- **P1 — canonical-fixpoint bijection:** `let c = canonicalize(d.clone())?; let (bytes, total_bits) = encode_payload(&c)?; decode_payload(&bytes, total_bits)? == c` AND `encode_payload(&d)? == encode_payload(&c)?` (the `(Vec<u8>,usize)` tuples byte-equal — pins the encoder's internal canonicalization against the explicit one). Note `decode_payload` is 2-arity `(&[u8], usize)`; destructure the encode tuple (do NOT transcribe `decode_payload(encode_payload(..))` literally — R0 M4).
- **P2 — canonicalize-is-normalizer (F4-class catcher):** over deliberately-scrambled descriptors (non-canonical indices + populated `Divergent` paths + multiple populated TLV maps simultaneously), assert `encode_payload(&d)? == encode_payload(&canonicalize(d.clone())?)?` byte-equal AND `{ let c = canonicalize(d.clone())?; let (bytes, total_bits) = encode_payload(&d)?; decode_payload(&bytes, total_bits)? == c }` (destructure the `(Vec<u8>,usize)` tuple — do NOT nest the calls; R1 I-R1-B). This hits the #1 bug surface (`canonicalize.rs` permutation lockstep), currently covered only for hand-picked n≤3.
- **P3 — decode panic-freedom:** arbitrary `&[u8]` → `decode_payload`; arbitrary `&str` → `decode_md1_string`, `chunk::reassemble`; never panic (return `Ok`/`Err`). **R0 I1 — `decode_payload(bytes, total_bits)` has a `debug_assert!(total_bits <= bytes.len()*8)` (`bitstream.rs:114`, via `BitReader::with_bit_limit`), and integration tests run in debug. So the `decode_payload` arm MUST pass `total_bits = bytes.len()*8` (or draw it as `total_bits.min(bytes.len()*8)`) — fuzzing `total_bits` freely would panic, falsely failing P3.** The `&str` arms (`decode_md1_string`/`reassemble`) derive their bit count internally → no such hazard, stay free-form. Folds in the unencodable-input contract: `Some(vec![])` TLVs / empty TLV entries return a typed `Err` (`EmptyTlvEntry`, `error.rs:144`), never a panic.
...
### §3.3 Notes (architect + R0)
- **R0 I4 — `tests/common/mod.rs` MUST start with `#![allow(dead_code)]` (and `#![allow(unused_imports)]`).** The shared module is compiled fresh into EACH integration test binary that does `mod common;`; helpers unused by a given binary become `dead_code` warnings → CI `-D warnings` red. (The mk-codec cycle hit exactly this.)
...
| T3c | delete below the 13-symbol checksum length (deterministic out-of-band) | `matches!(err, Error::Codex32DecodeError(_))` — **R0 M2: do NOT pin the `"too short"` substring.** `bch_verify_regular` runs FIRST (`codex32.rs:115`) and almost always fails ("BCH checksum verification failed") before the `symbols.len()<13` "too short" check (`:122`) is reached; the too-short message is effectively unreachable. Assert the broader `Codex32DecodeError` variant. |
...
`encode.rs:16-17,37-41,65-92` (PartialEq, kiw, internal canonicalize); `decode.rs:26,56,74-82` (kiw dup, placeholder validate, rollback); `canonicalize.rs:168,955` (sole normalizer, fixpoint test); `validate.rs:30,164,221` (non-canonical reject, forbidden-leaf, xpub bytes); `bch_decode.rs:12,416` (13-symbol checksum, deg>4); `chunk.rs:175-179,305,348/351/362/382,492,524-570` (csid width, reassemble, cross-chunk branches, decode_with_correction, correction loop + re-verify guard); `codex32.rs:18,86-91,114-126` (checksum width, padding, hard verify, too-short); `use_site_path.rs:43-45`; `tree.rs` (Node/Body/Tag + tr taptree wire). Consumer: `mnemonic-toolkit/crates/mnemonic-toolkit/src/repair.rs:962,1028,1043`.
