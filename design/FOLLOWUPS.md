...
- **This repo's verified findings (4):**
  - **[obs, filed 2026-06-10 by the toolkit v0.53.3 HRP-case cycle]** `md-codec-accepts-mixed-case-bip173-leniency` — md-codec decodes per-char-lowercased, so it accepts MIXED-case md1 strings that BIP-173 rejects (mk-codec and codex32 both reject Mixed). Toolkit characterization-tests the leniency (cli_hrp_case_insensitive.rs) rather than "fixing" it toolkit-side; decide codec-side whether to add a consistent-case gate. (`crates/md-codec/src/codex32.rs:92-119 unwrap_string per-char lowercase; chunk.rs:429-431`)
