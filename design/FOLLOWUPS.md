...
- **Status:** open
- **Tier:** v0.3-blocker — ELEVATED from v0.3-nice-to-have per Phase 5 code-quality reviewer pushback. Reasoning: round-trip self-consistency tests cannot detect polymod-constant drift if `bch_create_checksum_*` and `bch_verify_*` drift together (e.g., wrong polynomial constant would silently corrupt both). The original pinned-checksum tests caught that class of regression; the converted form does not. The Python-reference values for HRP `"md"` with the same input data (`[0..7]` and `[0..15]`) can be computed offline and hard-coded before v0.3.0 ships.
