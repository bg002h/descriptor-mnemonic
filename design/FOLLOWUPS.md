...
- **Actions on close:** (1) watch for the first release containing #953 (14.0 or a 13.x point release; optionally ask upstream for a 13.x backport — 13.1.0 emits unparseable descriptors, a data-interchange hazard for any 13.x consumer); (2) on a miniscript bump past the fix: the flip cell goes red → invert it, restore the depth-2 arm in `t_tr_tree`, resolve this entry.
- **Status:** `open`
- **Tier:** `upstream`
