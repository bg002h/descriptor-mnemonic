...
- **Status:** ⚠ **phase 1 PARTIAL — status corrected 2026-06-24 (was incorrectly "resolved").** md-codec-v0.9.1 shipped `rust-toolchain.toml` (rustc 1.85.0) + `.cargo/config.toml` `[profile.release]` `codegen-units = 1` + `strip = "symbols"` — BUT the **`--remap-path-prefix`** that phase-1's own plan (above) lists as the key path-reproducibility win was **OMITTED** from the shipped config (grep-verified absent @ origin/main 2026-06-24). CONSEQUENCE — **empirically PROVEN** by the toolkit-led `reproducible-builds-musl` deep cycle-prep (@ `43298598`): two release builds from byte-identical source at DIFFERENT paths are **NOT byte-identical** (root cause = absolute build-path leak in `.rodata` panic-`Location` strings — also a **`$HOME` PRIVACY leak** baked into every shipped binary); adding the remap makes them byte-identical on the current 1.85.0 pin. So phase-1's path-reproducibility is **NOT delivered**. The remap + full bit-for-bit reproducibility (incl. the secp256k1-sys `cc`-under-musl validation + the hermetic env that was phase-2) is now being delivered by the constellation **`reproducible-builds-musl`** cycle (toolkit-led; deep cycle-prep done + brainstorm R0 in flight) — this slug's remaining work folds into it.
- **Tier:** ⚠ phase 1 INCOMPLETE (remap omitted → path-leak OPEN); phase 2 open. Both fold into the toolkit-led `reproducible-builds-musl` cycle. **Companion:** `mnemonic-toolkit/cycle-prep-recon-reproducible-builds-musl.md` (the empirical proof + the recipe).
