...
### `toolkit-check-pkk-non-tap-non-canonical` — md-cli conformant / mnemonic-toolkit walker keeps `Check(PkK/PkH)` in wsh/sh (companion)
...
- **Fix (deferred, toolkit-side, R0-gated, separate cycle):** the TOOLKIT drops its `tap_context` gate (collapse to bare in non-tap too) and inverts `walk_check_kept_in_non_tap_context`. No md-cli change. Tracked primary in `mnemonic-toolkit/design/FOLLOWUPS.md` (`toolkit-check-pkk-non-tap-non-canonical`).
- **Companion:** mnemonic-toolkit `design/FOLLOWUPS.md` (the primary, toolkit-pointed entry). See also the related `v2-design-questions` item 12 below ("Walker context-dependent `Check(PkK)` mangling") — that item described the OLD md-codec wire-design question (resolved by the v0.30 cycle, which made md-cli's walker conformant); THIS entry is the live, toolkit-side residual.
- **Tier:** `cross-repo` (companion; the actionable fix is toolkit-side).
...
- **What:** This entry catalogs design choices that v0.x ships with for good reason (path dependence, accumulated constraints, organic evolution) but that a clean-slate v2.0 redesign would likely re-evaluate. Capturing them now prevents future sessions from reinventing the analysis.
  ...
  **Post-v0.11 retrospective batch (added 2026-05-10).** The v0.4.5 BIP bitstream-native rewrite cycle dug into the v0.11+ wire format in detail and surfaced these additional concrete pain points worth carrying into a v2 retrospective. Items 1-8 above pre-date the v0.11 wire-format reset; items 9-13 below are framed against the shipped v0.11+ wire.
  ...
  12. **Walker context-dependent `Check(PkK)` mangling.** In tap context, `walk_miniscript_node` collapses `Terminal::Check(Terminal::PkK(K))` to bare `Tag::PkK` on the wire (because BIP 388 tap-leaf canonical form is bare `pk_k`). In segwitv0 (wsh) context, the same `Check(PkK)` is preserved as `Tag::Check` wrapping `Tag::PkK`. Same miniscript input produces different wire shapes depending on the top-level descriptor. A v2 design could normalize uniformly at the walker (always collapse `Check(PkK)` and `Check(PkH)` to bare key tags) and shift the context-aware decoration to the renderer. Trades wire complexity for renderer complexity, but unifies the wire-shape audit surface. **(Cross-ref, 2026-06-12: the v0.30 cycle RESOLVED this for md-cli's walker — `template.rs:607` now collapses unconditionally per SPEC §5.1. The live RESIDUAL is the `mnemonic-toolkit` CLI walker, which still keeps `Check(PkK)` in wsh/sh — see the `toolkit-check-pkk-non-tap-non-canonical` companion entry above; the actionable fix is toolkit-side.)**
