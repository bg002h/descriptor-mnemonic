...
- **TODO — file a GitHub issue someday (user-requested 2026-06-11):** open a rust-bitcoin/rust-miniscript issue requesting a **13.x backport of #953** — frame it as "13.1.0 `Descriptor::Display` emits unparseable strings for unbalanced (≥3-leaf) taptrees; fixed on master by #953, please backport to the 13.x line." Not urgent (we route around the string form; the flip cell + depth≤1 generator constraint contain our exposure), hence "someday" — but worth doing because ANY 13.x consumer that round-trips taproot descriptors through strings is silently broken. Include the minimal repro from this entry. NOT a duplicate bug report (the bug is fixed on master); it is a release/backport request.
- **Actions on close:** (1) file the backport-request issue (above) when convenient; (2) watch for the first release containing #953 (14.0 or a 13.x point release); (3) on a miniscript bump past the fix: the flip cell goes red → invert it, restore the depth-2 arm in `t_tr_tree`, resolve this entry.
- **Status:** `open`
- **Tier:** `upstream`
